...
def generate_links(user_path):
    ...
    a_target = " target=\"_parent\""
    homelink = """
    <a target="_parent" href="index.html">Overview</a>
    <hr>
    """
    css = """
    <style>
    ...
    file.write(str(homelink)) #

    for i in range(0,len(list_files)): #list all files using array length

        if(list_files[i][0] == '.'): #if first letter has a dot
            pass
        elif(list_files[i].__contains__(html_name)): #skip menu.html
            pass
        elif(list_files[i].__contains__("index.html")):#skip index.html
            pass
        elif( list_files[i].__contains__('.html') ):
